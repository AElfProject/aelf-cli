...
internal class MnemonicService : IMnemonicService, ITransientDependency
{
    ...
    //
    //     if (words.Length % 3 != 0) throw new FormatException(Bip39Constants.InvalidMnemonic);
    //
    //     var bits = string.Join("", words.Select(word =>
    //     {
    //         var index = Array.IndexOf(wordlist, word);
    //         if (index == -1) throw new FormatException(Bip39Constants.InvalidMnemonic);
    //
    ...
    {
        var mnemonicBytes = Encoding.UTF8.GetBytes(mnemonic.Words.ToString()?.Normalize(NormalizationForm.FormKD) ?? string.Empty);
