...
public static class RenameHelper
{
    ...
    private static void RenameDirectoryRecursively(IEnumerable<FileEntry> entries, string placeHolder, string name)
    {
    ...
    private static void RenameAllFiles(IEnumerable<FileEntry> entries, string placeHolder, string name)
    {
        foreach (var entry in entries.Where(e => !e.IsDirectory))
        {
            if (entry.Name.Contains(placeHolder))
    ...
    private static void ReplaceContent(IEnumerable<FileEntry> entries, string placeHolder, string name)
    {
        foreach (var entry in entries.Where(e => !e.IsDirectory))
        {
