...
public class TopOfOasisCommand : IAElfCommand, ITransientDependency
{
    ...
    public TopOfOasisCommand(ITopOfOasisService topOfOasisService, IUserContext userContext,
        ...
        try
        {
            ...
            switch (commandLineArgs.Target?.ToLower())
            {
                case "upload-project":
                    ...
                    {
                        var projectId =
                            StringValue.Parser.ParseFrom(ByteArrayHelper.HexStringToByteArray(result.ReturnValue));
