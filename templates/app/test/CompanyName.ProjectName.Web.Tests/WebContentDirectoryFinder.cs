...
public static class WebContentDirectoryFinder
{
    public static string CalculateContentRootFolder()
    {
        var domainAssemblyDirectoryPath =
            Path.GetDirectoryName(typeof(ProjectNameDomainModule).Assembly.Location);
        if (domainAssemblyDirectoryPath == null)
        {
            throw new Exception(
                $"Could not find location of {typeof(ProjectNameDomainModule).Assembly.FullName} assembly!");
        }
        ...
        {
            while (!DirectoryContains(directoryInfo.FullName, "CompanyName.ProjectName.Web.csproj", SearchOption.AllDirectories))
            {
            ...
            var webProject = Directory.GetFiles(directoryInfo.FullName, string.Empty, SearchOption.AllDirectories)
                .First(filePath => string.Equals(Path.GetFileName(filePath), "CompanyName.ProjectName.Web.csproj"));
